...
rag/pipeline.py
────────────────
Main NL2SQL RAG Pipeline — orchestrates the full flow:

    Natural Language Query
         │
         ▼
    [1] SchemaRetriever  → vector search + reranking → relevant schemas
         │
         ▼
    [2] SQLGenerator     → LLM + schema context      → SQLGenerationResult
         │
         ▼
    [3] SQL Executor     → run against target DB      → rows / error
         │
         ▼
    [4] Self-healing     → on error, refine & retry   → corrected result
         │
         ▼
    PipelineResult       → returned to API layer

The pipeline also:
- Indexes target database schemas into the vector store on first run.
- Logs every request to Postgres via QueryHistory.
...
class NL2SQLPipeline:
    ...
    def query(
        ...
        # ── Step 3: Execute SQL with self-healing ─────────────────────────────
